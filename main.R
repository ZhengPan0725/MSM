rm(list = ls()) 
setwd("~/Desktop/ZhengPan/workspace/NUIST/John/MSM/")  
...
learner <- lrn(learner_to_use,
               ...
               gamma = exp(-4),
               tolerance = exp(-2))
rr = resample(task, learner, cv10)
acc = rr$score(msr("classif.acc"))
predictions_train <- rr$prediction(predict_sets = 'train')
predictions_test <- rr$prediction(predict_sets = 'test')
measure = msr("classif.acc")
measure$score(predictions_test)
predictions_test$confusion
library(cvms)
library(tibble)
plot_confusion_matrix(as.tibble(predictions_test$confusion), 
                      target_col = "truth", 
                      prediction_col = "response",
                      counts_col = "n",
                      palette = "Oranges")
ggsave(filename = '~/desktop/test.jpeg')
...
#==== Constants ===========
##==== not changable ====
st1 <- read_csv(file = '../胡文广/A4BX6/rawdata/A4BX6train_and_test_fill1.csv')
abbs <- read_csv(file = '../胡文广/A4BX6/rawdata/Abbs.csv')
st <- clean_up(df = st1, abbs = abbs)
features <- st[, -ncol(st):(-ncol(st) + 1)] 
fea_new <- fea_slc_bycor(features, corr = 0.95)
# fea_new <- features
# fea_new <- features[, setdiff(colnames(features), colnames(fea_new))]
means <- apply(fea_new, 2, mean)
sds <- apply(fea_new, 2, sd)
fea_new <- scale(fea_new, center = means, scale = sds)
bandgapd <- cbind(fea_new, bandgap = st$lumo - st$homo) 
...
library(cvms)
library(tibble)
plot_confusion_matrix(as_tibble(predictions_test$confusion), 
                      target_col = "truth", 
                      prediction_col = "response",
                      counts_col = "n",
                      palette = "Oranges") + 
  labs(title = 'train without selected feature')
ggsave(filename = 'res_new/ML/ML_without_feature.jpeg')
...
plot_confusion_matrix(as_tibble(predictions_test$confusion), 
                      ...
                      counts_col = "n",
                      palette = "Oranges") + 
  labs(title = 'train with selected feature')
ggsave(filename = 'res_new/ML/ML_with_feature.jpeg')
